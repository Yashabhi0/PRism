...
export default function SignUpPage() {
  ...
  const handleGitHub = async () => {
    ...
    await supabase.auth.signInWithOAuth({
      provider: "github",
      options: {
        redirectTo: `${siteUrl}/auth/callback?next=${encodeURIComponent("/repos")}`,
      ...
      >
        <div className="border border-border cyber-chamfer bg-card overflow-hidden shadow-void">
          ...
          <div className="flex items-center gap-2 px-5 h-8 bg-muted border-b border-border">
            ...
            <span className="w-2 h-2 rounded-full bg-accent-secondary opacity-70" />
          ...
          <div className="p-8">
            ...
            <div className="flex flex-col items-center text-center mb-8 gap-4">
              <div className="w-14 h-14 cyber-chamfer border border-accent-secondary/30 bg-background flex items-center justify-center hover:border-accent-secondary hover:shadow-neon-secondary transition-all duration-150">
                <Cpu className="w-6 h-6 text-accent-secondary/70" strokeWidth={1.5} />
              </div>
              <div>
                <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-accent-secondary">
                  // New Identity
                </span>
                ...
                <p className="font-mono text-xs text-muted-foreground mt-2">
                  Scaffold your developer review dashboard.
                </p>
              </div>
            </div>
            ...
            <div className="mb-6 grid grid-cols-1 gap-2">
              {[
                { icon: Shield, text: "Automated security scanning on every PR" },
                { icon: Zap,    text: "Sub-2s analysis with multi-agent debate" },
                { icon: Cpu,    text: "Full GitHub integration, zero config" },
              ].map(({ icon: Icon, text }) => (
                <div key={text} className="flex items-center gap-3 border border-border/50 cyber-chamfer-sm bg-background/50 px-4 py-2.5">
                  <Icon className="w-3.5 h-3.5 text-accent-secondary/60 shrink-0" strokeWidth={1.5} />
            ...
            <div className="mb-6">
              <GitHubButton
                ...
                label={loading ? "Redirecting to GitHub..." : "Sign Up with GitHub"}
                disabled={loading}
              />
            </div>
            ...
            <p className="text-center font-mono text-xs text-muted-foreground">
              ...
              <Link href="/signin" className="text-accent hover:text-accent/80 underline underline-offset-4 transition-colors focus-visible:outline-none">
                Sign in
              </Link>
            </p>
          </div>
